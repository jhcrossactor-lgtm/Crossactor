...
**送る前に全画像を正規化する。** RGB の PNG に変換し、EXIF の回転を画素に焼き込み、
長辺を `max_input_edge`（既定 2048px）以下に縮小してから送る。iPhone 写真の
CMYK・16bit・RGBA・EXIF回転・巨大サイズ・HEIC中身の `.JPG` はすべてこれで通る。
変換結果は `stage1/_prepared/` に置かれる（成果物のコピー対象外）。
`python run.py check` で各素材のモード・寸法・EXIF回転の有無を事前に確認できる。

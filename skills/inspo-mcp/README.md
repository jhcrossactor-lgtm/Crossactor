...
- **2026-09-16：3スキル（promo-design / event-lp / seo-lp）とも claude.ai 側で反映済み**（ほせもやん実施）
- **同日：リポジトリの `CLAUDE.md` に「デザイン実装前の必須手順」を追加済み**
  スキルが発火しないケース（例「このヒーローの配色だけ直して」「管理画面のUI作って」）を塞ぐため
